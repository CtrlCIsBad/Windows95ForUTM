...
Windows 95 is a completely obsolete operating system. It lacks modern security protocols, firewalls, and encryption standards. Enabling networking inside this Virtual Machine (VM) exposes the guest environment to severe security vulnerabilities. 
...
## 🌐 How to Enable Networking (Advanced Users Only)
...
1. Open **UTM** and select your Windows 95 virtual machine.
2. Open the VM **Settings** configuration panel.
3. Navigate to the **Network** tab and click **New...** to add a hardware interface.
4. Set the Network Mode to **Shared Network (NAT)** (Highly recommended over Bridged for isolation).
5. Change the Emulated Network Card device type to **ne2k_pci**.
6. Save the settings and boot the VM. Windows 95 will detect the legacy Realtek/Novell compatible network card.
